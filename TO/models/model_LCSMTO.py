...
from TO.visual_lcsmto.plot_latenth import plot_sep
...
from TO.visual_lcsmto import plot_ls
...
class LCSMTO(GPR):
    ...
    def __init__(
        ...
        # self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # checks whether a GPU is available
        ...
        self.Nx = None ####### ADDED
        self.Ny = None ####### ADDED
        self.mask_col = None####### ADDED
        self.mask_tensor = None####### ADDED
        ...
        if self.basis=='single':
            ...
            # self.mean_module=LinearMean_with_prior(input_size=basis_function_size, batch_shape=torch.Size([]), bias=True)
            ...
            ############################################### One NN for ALL
            if len(qual_index) == 0:
                setattr(self,'mean_module_NN_All', FFNN_for_Mean(self, input_size= train_x.shape[1], num_classes=5,layers =NN_layers_base, name = str('mean_module_'+str(i)+'_'))) 
            else:
        ...
        #     setattr(self,'mean_module_NN_All', NetworkM4(input_dim = train_x.shape[1], output_dim=3, layers = NN_layers_base)) 
            ...
            network =  NetworkM4_fused(input_dim = train_x.shape[1], output_dim=4, layers = self.NN_layers_base, activation = self.activation)
            encoder = Encoder(
        ...
        # self.covar_module.raw_outputscale.data = torch.tensor(0.541)  # Set the desired value
        self.covar_module.raw_outputscale.data = torch.tensor(-1.0)  # Set the desired value
        self.covar_module.raw_outputscale.requires_grad = False  # Fix the hyperparameter
        
        # self.covar_module.outputscale.data = torch.tensor(0.0)  # Set the desired value
        # self.covar_module.outputscale.requires_grad = False  # Fix the hyperparameter
...
#################### Multiple bases (General Case) ####################################  
        def multi_mean(x,x_forward_raw):
            ...
            return mean_x 
    ##########################################################################################
        # if self.name_output == 'u' or self.name_output == 'v':
        #     x.requires_grad_(True)
        #     psi = multi_mean(x,x_forward_raw)[:,0].reshape(-1)
        #     psi_derivatives = torch.autograd.grad(psi, x, torch.ones_like(psi), True, True)[0]
        #     if self.name_output == 'u':
        #         mean_x = psi_derivatives[:,1]
        #     elif self.name_output == 'v':
        #         mean_x = -psi_derivatives[:,0]
    ...
    def visualize_latent_position(self,lv_columns=None):                             ####################### Here this function is defined to be used for imputation purpouses
        if len(self.qual_kernel_columns) > 0:
            for i in range(len(self.qual_kernel_columns)):
                zeta = self.zeta[i]
                A = self.A_matrix[i]
...
#########################                         Amin :                I made all bias=False tpo check for a case, RRRRRReturrrn them back
