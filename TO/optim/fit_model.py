...
import pandas as pd
import os
from TO.utils import  plot_loss_history,plot_density_and_velocity_fields
import datetime
...
checkpoints=[10,4000,20000,40000,50000]
lambdaa = 0.1
...
def loss_volume(y, gamma=0.5):
    ...
    # Compute the mean of y (which corresponds to the integral over the domain)
    mean_y = torch.mean(y)
    
    # Compute the maximum between 0 and (mean_y - gamma)
    return torch.square(torch.max(torch.tensor(0.0), mean_y - gamma))
...
def calculate_loss_multioutput(model_list, iteration, diff_method='Numerical', title="default",ratio=1.0,volume_fraction=1/3):
    """
    ...
    gamma = volume_fraction # next((val for key, val in gamma_values.items() if key in title), 0.5)
    vol_loss = loss_volume(z_all[:, 3:], gamma=gamma)* w_4

    if title.find('doublepipe')!=-1:
        if iteration<3e4:
            if vol_loss< 1e-2:
                vol_loss*=0
...
def find_TO(
    ...
    # Training loop
    for j in tqdm(range(num_iter + 1), desc='Epoch', position=0, leave=True):
        optimizer.zero_grad()

        # Calculate losses
        loss_pde1, loss_pde2, loss_pde3, loss_dp, vol_loss = calculate_loss_multioutput(
            model_list, j, diff_method=diff_method, title=title,ratio=ratio,volume_fraction=volume_fraction
        )
